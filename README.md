<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=ff4655&height=220&section=header&text=Sathvik%20N%20G&fontSize=68&fontColor=ece8e1&animation=fadeIn&fontAlignY=40&desc=Senior%20Full%20Stack%20AI%20Engineer&descAlignY=62&descColor=ffffff&descSize=17" width="100%" />

<br/>

[![Twitter](https://img.shields.io/badge/X-@sathvik182-ff4655?style=flat-square&logo=x&logoColor=white)](https://x.com/sathvik182)
[![GitHub](https://img.shields.io/badge/GitHub-sathvikng-161b22?style=flat-square&logo=github&logoColor=white)](https://github.com/sathvikng)
[![Email](https://img.shields.io/badge/Email-sathvik.182@gmail.com-ff4655?style=flat-square&logo=gmail&logoColor=white)](mailto:sathvik.182@gmail.com)
[![Location](https://img.shields.io/badge/Bengaluru%2C%20India-0a0f16?style=flat-square&logo=googlemaps&logoColor=white)](https://github.com/sathvikng)
[![Profile Views](https://komarev.com/ghpvc/?username=sathvikng&color=ff4655&style=flat-square&label=Profile+Views)](https://github.com/sathvikng)

</div>

---

## `> whoami`

Growing up in Bangalore, my weekends were spent at **SP Road** — the city's electronics market — learning to build PCs from scratch with the vendors. That hands-on curiosity with hardware pulled me into software. I discovered I could make the games I played, built my first 2D games in Lua, and never stopped building.

Today I'm a **founding engineer at Vaia**, owning the full AI stack across finance and sustainability. Before that, two years at **Accenture** embedded with **Microsoft AI Research** and **Avanade**, shipping production AI for Fortune 500 clients. Before that, **Nokia Bell Labs** during undergrad.

The instinct is always the same: see the inefficiency, understand the system, build the better version.

---

## `> career --timeline`

```
Nokia Bell Labs      Undergrad Research Intern     AI Research
Accenture            AI Engineer (2 yrs)           Microsoft AI Research · Avanade · Fortune 500
Vaia                 Founding Engineer              Full AI Stack · Finance & Sustainability
```

---

## `> ls projects/`

<table>
<tr>
<td width="50%" valign="top">

### XFinBench
No rigorous benchmark existed to measure how well LLMs reason in finance. So I built one. Tests model reasoning quality against domain-specific financial logic at scale — not just accuracy, but depth of understanding.

</td>
<td width="50%" valign="top">

### Real Estate Sustainability Platform
Sustainability teams were manually compiling emissions data across dozens of properties every quarter. Built an automated end-to-end pipeline that handles the entire thing. They don't do it by hand anymore.

</td>
</tr>
</table>

---

## `> cat research.log`

| Publication | Venue |
|---|---|
| Pathfinding Algorithms: Dijkstra vs A\* with Euclidean & Manhattan Heuristics | 📗 **Springer Nature** |
| Brain Tumor Classification via Transfer Learning + AutoML | 📘 **IEEE** |

> *The second one was built when AutoML was barely a concept. It went from a Pygame simulation I made to understand pathfinding, to a paper, because my research guide walked it to the Dean.*

---

## `> cat awards.log`

🏆 &nbsp; **NASA International Space Apps Challenge** — *Galactic Problem Solver* &nbsp; `2019`

> Built a live 3D solar system with real orbital data from NASA's Horizons API. Actual planetary positions, orbital periods, and trajectories — fully interactive. First year of college.

🏆 &nbsp; **Microsoft Hackathon** — *Winner*

---

## `> tech --stack`

**AI / ML**

![Python](https://img.shields.io/badge/Python-0d1117?style=flat-square&logo=python&logoColor=3776AB)
![PyTorch](https://img.shields.io/badge/PyTorch-0d1117?style=flat-square&logo=pytorch&logoColor=EE4C2C)
![TensorFlow](https://img.shields.io/badge/TensorFlow-0d1117?style=flat-square&logo=tensorflow&logoColor=FF6F00)
![LangChain](https://img.shields.io/badge/LangChain-0d1117?style=flat-square&logo=langchain&logoColor=1C3C3C)
![OpenAI](https://img.shields.io/badge/OpenAI-0d1117?style=flat-square&logo=openai&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-0d1117?style=flat-square&logo=huggingface&logoColor=FFD21E)

**Full Stack**

![TypeScript](https://img.shields.io/badge/TypeScript-0d1117?style=flat-square&logo=typescript&logoColor=3178C6)
![Next.js](https://img.shields.io/badge/Next.js-0d1117?style=flat-square&logo=nextdotjs&logoColor=white)
![React](https://img.shields.io/badge/React-0d1117?style=flat-square&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-0d1117?style=flat-square&logo=nodedotjs&logoColor=339933)
![Three.js](https://img.shields.io/badge/Three.js-0d1117?style=flat-square&logo=threedotjs&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-0d1117?style=flat-square&logo=fastapi&logoColor=009688)

**Cloud & Infrastructure**

![AWS](https://img.shields.io/badge/AWS-0d1117?style=flat-square&logo=amazonaws&logoColor=FF9900)
![Azure](https://img.shields.io/badge/Azure-0d1117?style=flat-square&logo=microsoftazure&logoColor=0078D4)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-0d1117?style=flat-square&logo=postgresql&logoColor=4169E1)
![Docker](https://img.shields.io/badge/Docker-0d1117?style=flat-square&logo=docker&logoColor=2496ED)

---

## `> github --stats`

<div align="center">

<br/>

<img width="62%" src="https://streak-stats.demolab.com?user=sathvikng&theme=dark&hide_border=true&background=0d1117&ring=ff4655&fire=ff4655&currStreakLabel=ff4655" />

</div>

---

<div align="center">

*If you're building something in AI, finance, or sustainability — I'd genuinely love to hear about it.*

[![Twitter](https://img.shields.io/badge/Reach%20out%20on%20X-ff4655?style=for-the-badge&logo=x&logoColor=white)](https://x.com/sathvik182)

<img src="https://capsule-render.vercel.app/api?type=waving&color=ff4655&height=120&section=footer&animation=fadeIn" width="100%" />

</div>
